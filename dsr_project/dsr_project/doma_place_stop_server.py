...
def perform_task_loop():
    ...
    while rclpy.ok():
        ...
        try:
            # [핵심] movel, movej, check_motion
            from DSR_ROBOT2 import (
                movel, movej, check_motion,
                set_digital_output,
                posx, posj,
            ...
            if not wait_with_feedback("도마 내려놓기...", 1.5, 60.0, 70.0): raise Exception("작업 중단")
            ...
            if not wait_with_feedback("안전 거리 확보 중...", 2.0, 75.0, 80.0): raise Exception("작업 중단")
            ...
            if not wait_with_feedback("복귀 자세 정렬 중...", 2.0, 80.0, 85.0): raise Exception("작업 중단")
            ...
            if not wait_with_feedback("복귀 중 (고속)...", 2.0, 85.0, 90.0): raise Exception("작업 중단")
            ...
            # 마무리
            movej(posj(-0.8, 16.18, 69.13, 1.06, 61.25, 0.71), vel=VELOCITY, acc=ACC, radius=0.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("작업 중단")
            ...
            # if not wait_for_motion(): raise Exception("작업 중단")
            if not wait_with_feedback("홈 위치로 복귀...", 2.0, 95.0, 100.0): raise Exception("작업 중단")
