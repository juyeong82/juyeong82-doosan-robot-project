...
def perform_task_loop():
    ...
    while rclpy.ok():
        ...
        try:
            # [핵심] movel, movej, check_motion 추가
            from DSR_ROBOT2 import (
                movel, movej, check_motion, # 비동기 명령
                set_digital_output,
                posx, posj,
            ...
            def gripper(switch: int):
                ...
                # time.sleep(1.0) # 대기 시간은 main flow에서 제어
            ...
            # 도마잡기 전
            movej(posj(-5.50, 5.92, 76.95, -1.54, 66.92, 0.00), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            # if not wait_for_motion(): raise Exception("작업 중단")
            if not wait_with_feedback("도마 위치로 이동 중...", 2.0, 5.0, 15.0): raise Exception("중단")

            movej(posj(-5.31, 26.53, 60.17, 1.18, 64.83, 0.12), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            # if not wait_for_motion(): raise Exception("작업 중단")
            if not wait_with_feedback("접근 자세 제어 중...", 2.0, 15.0, 25.0): raise Exception("중단")

            # 도마 잡기 위치
            movej(posj(-2.21, 31.13, 61.13, 1.06, 59.34, 0.71), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            # if not wait_for_motion(): raise Exception("작업 중단")
            if not wait_with_feedback("파지 위치 정렬 중...", 1.5, 25.0, 30.0): raise Exception("중단")

            gripper(0)
            time.sleep(1.0)
            pub_feedback("도마 파지 끝", 35.0)
            ...
            # if not wait_for_motion(): raise Exception("작업 중단")
            if not wait_with_feedback("작업대로 이동 중...", 3.0, 35.0, 60.0): raise Exception("중단")
            ...
            if not wait_with_feedback("작업대 위 정렬 중...", 1.5, 60.0, 65.0): raise Exception("중단")

            # (65% -> 75%) - 연속동작 묶음 1
            movej(posj(-24.57, -15.92, 104.11, 34.45, 52.34, -45.38), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("중단") # 짧은 동작은 wait_for_motion으로 넘김
            
            movej(posj(-50.42, 0.29, 108.69, 64.81, 80.28, -97.76), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_with_feedback("자세 변경 중 (1/2)...", 2.0, 65.0, 75.0): raise Exception("중단")
            
            # (75% -> 85%) - 연속동작 묶음 2
            movej(posj(-50.13, 27.81, 92.24, 64.66, 104.44, -124.11), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE); wait_for_motion()
            movej(posj(-45.01, 50.34, 67.31, 63.27, 124.55, -128.64), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE); wait_for_motion()
            
            movej(posj(-45.39, 57.76, 68.16, 63.27, 123.92, -135.78), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_with_feedback("자세 변경 중 (2/2)...", 2.0, 75.0, 85.0): raise Exception("중단")

            movej(posj(-45.31, 61.34, 71.30, 59.33, 121.63, -141.12), vel=VELOCITY, acc=ACC, radius=0.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("중단")

            # 5. 안착 (85% -> 90%)
            movel(posx(465.54, -315.23, 71.47, 90.00, 90.00, 0.00), vel=VELOCITY, acc=ACC, radius=0.00, ref=0, mod=DR_MV_MOD_ABS, ra=DR_MV_RA_DUPLICATE)
            if not wait_with_feedback("작업대 안착 중...", 1.5, 85.0, 90.0): raise Exception("중단")

            gripper(1)
            time.sleep(1.0)
            pub_feedback("도마 안착 끝", 92.0)

            # 뒤로 빼기 및 이동을 위한 정렬
            movel(posx(0.00, -100.00, 0.00, 0.00, 0.00, 0.00), vel=VELOCITY_fast, acc=ACC_fast, radius=30.00, ref=0, mod=DR_MV_MOD_REL, ra=DR_MV_RA_DUPLICATE)
            # if not wait_for_motion(): raise Exception("작업 중단")
            if not wait_with_feedback("안전 거리 확보 중...", 1.5, 92.0, 95.0): raise Exception("중단")

            movej(posj(-49.51, 54.09, 28.09, 44.63, 130.44, -131.66), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("작업 중단")
