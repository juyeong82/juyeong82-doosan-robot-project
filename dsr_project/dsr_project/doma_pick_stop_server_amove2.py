...
def perform_task_loop():
    ...
    while rclpy.ok():
        ...
        try:
            # [핵심] amovel, amovej, check_motion 추가
            from DSR_ROBOT2 import (
                amovel, amovej, check_motion, # 비동기 명령
                set_digital_output,
                posx, posj,
            ...
            # [Helper] 피드백 퍼블리시 함수 추가
            def pub_feedback(msg_str, percent):
                feedback_msg.feedback_string = msg_str
                feedback_msg.current_stroke = 0 # Pick은 stroke 의미 없음
                feedback_msg.progress_percentage = float(percent)
                g_current_goal_handle.publish_feedback(feedback_msg)
                logger.info(f"[Progress] {percent}% - {msg_str}")
            ...
            # 2. 접근 (Approach)
            pub_feedback("도마 픽킹 위치로 접근 중...", 20.0)

            # 2. Pick 시퀀스 (amovej, amovel 적용)
            ...
            # 도마잡기 전
            amovej(posj(-5.50, 5.92, 76.95, -1.54, 66.92, 0.00), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("작업 중단")

            amovej(posj(-5.50, 5.92, 76.95, -1.54, 66.92, 0.00), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("작업 중단")

            pub_feedback("접근 정밀 제어 중...", 30.0)
            amovej(posj(-5.31, 26.53, 60.17, 1.18, 64.83, 0.12), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("작업 중단")

            # 도마 잡기 위치
            # 도마 잡기 위치
            pub_feedback("그리퍼 정렬 완료, 진입...", 40.0)
            amovej(posj(-2.21, 31.13, 61.13, 1.06, 59.34, 0.71), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("작업 중단")

            # 잡기
            pub_feedback("도마 파지 중...", 50.0)
            gripper(0)
            time.sleep(1.0)

            # 도마 작업대 위로 이동
            # 3. 이동 (Transport)
            pub_feedback("작업대로 이동 중...", 60.0)
            ...
            # 자세조정 및 내리기
            pub_feedback("작업대 배치 위치 정렬 중...", 70.0)
            ...
            amovej(posj(-45.31, 61.34, 71.30, 59.33, 121.63, -141.12), vel=VELOCITY, acc=ACC, radius=0.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("작업 중단")

            # 도착 (수평 정렬하며 이동)
            pub_feedback("작업대 안착 위치 도달...", 90.0)
            ...
            # 뒤로 빼기 및 이동을 위한 정렬
            # 4. 복귀
            pub_feedback("안전 거리 확보 후 복귀...", 95.0)
            ...
            amovej(posj(-49.51, 54.09, 28.09, 44.63, 130.44, -131.66), vel=VELOCITY, acc=ACC, radius=30.00, ra=DR_MV_RA_DUPLICATE)
            if not wait_for_motion(): raise Exception("작업 중단")

            logger.info("[Task Thread] 홈 복귀.")
            pub_feedback("Pick 작업 완료.", 100.0)
            amovej(home_pose_j, vel=30, acc=30)
            if not wait_for_motion(): raise Exception("작업 중단")
