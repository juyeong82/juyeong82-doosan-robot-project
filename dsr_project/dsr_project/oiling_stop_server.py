            ...
            if not task_running:
                ...
                try:
                    ...
                    def oiling_move(logger, num_strokes, y_step_size, move_x, vel_x, acc_x, vel_y, acc_y, vel_z, acc_z):
                        ...
                        amovel([-10,-65,0,0,0,0], vel=vel_y, acc=acc_y, ref=DR_BASE, mod=DR_MV_MOD_REL)
                        if not wait_with_feedback("오일링 2번째 줄 이동 중...", 1.5, 36.0, 40.0): return False

                        # 오일 보충
                        pose_temp, _ = get_current_posx()

                        amovel([492.91,233.52,430.63,46.64,179.98,46.65], vel=vel_y, acc=acc_y, t=2.5, ref=DR_BASE, mod=DR_MV_MOD_ABS)
                        #if not wait_with_feedback("오일 보충중...", 1.5, 36.0, 40.0): return False 
                        if not wait_for_motion(): return False

                        amovel([0,0,-85,0,0,0], vel=vel_z, acc=acc_z, ref=DR_BASE, mod=DR_MV_MOD_REL)
                        if not wait_for_motion(): return False
                        time.sleep(1)
                        amovel([0,0,85,0,0,0], vel=vel_z, acc=acc_z, ref=DR_BASE, mod=DR_MV_MOD_REL)
                        if not wait_for_motion(): return False

                        amovel(pose_temp, vel=vel_y, acc=acc_y, t=2.5, ref=DR_BASE, mod=DR_MV_MOD_ABS)
                        #if not wait_with_feedback("오일 보충중...", 1.5, 36.0, 40.0): return False 
                        if not wait_for_motion(): return False
