...
def perform_task_loop():
    ...
    while rclpy.ok():
        ...
        try:
            ...
            # [Helper] 비동기 대기 함수 (Pause 대응)
            def wait_for_motion():
                time.sleep(0.2) # 명령 전달 대기
                while check_motion() != 0: 
                    time.sleep(0.1) # Pause 신호 받을 틈
            ...
            # 2. 자세 잡기 (amovesj 적용 - 연속된 관절 이동 3개)
            ...
            # 3. 건조대 진입 및 배치 (amovesj 적용 - 연속된 관절 이동 6개)
            ...
            # [유지] 작업 스레드에서 succeed 호출 (원래 방식)
            if g_current_goal_handle: g_current_goal_handle.succeed()
